...
local cfg = {
    ...
    discord = "https://discord.gg/ekBW9RS4",
...
local function buildUI()
    ...
    Part2.Title.Text = "V5 FX-HUB DDS VEHICLE SPEED SCRIPT" 
...
-- MODIFIED VEHICLE BOOST SCRIPT WITH AUTO-SAVE & EASY TURN/STEERING
local function loadScript()
    print("Loading Vehicle Boost System with Auto-Save & Bypass System")
    
    -- HIGHER DEFAULTS WITH EASY TURN OPTION
    local DEFAULTS = {
        MAX_BOOST_FORCE = 300,
        BOOST_INCREMENT = 4.99,
        BOOST_INTERVAL = 0.04,
        EASY_TURN = true,  -- MODIFIED: Changed from STRAIGHT_BOOST to EASY_TURN
        BYPASS_REPORT = true,
    }
    ...
    local transparentButtons = false
    ...
    local steeringLocked = false -- NEW: Track if steering is currently locked
    ...
    -- Handle migration from old STRAIGHT_BOOST to EASY_TURN
    if boostConfig.STRAIGHT_BOOST ~= nil then
        -- Convert old STRAIGHT_BOOST to new EASY_TURN (inverted logic)
        boostConfig.EASY_TURN = not boostConfig.STRAIGHT_BOOST
        boostConfig.STRAIGHT_BOOST = nil -- Remove old key
    end
...
local function updateSpeedometerVisibility()
    if speedometer then
        if transparentButtons and boosting then
            speedometer.BackgroundTransparency = 1
            speedometer.TextTransparency = 1   -- 🔥 TAMBAH DI SINI

            for _, v in ipairs(speedometer:GetDescendants()) do
            ...
            speedometer.TextTransparency = 0   -- 🔥 TAMBAH DI SINI
...
local function setTransparentButtons(state, boostBtn, settingsBtn)
    transparentButtons = state
    local t = state and 1 or 0

    if boostBtn then
        boostBtn.BackgroundTransparency = t
        boostBtn.TextTransparency = t
    end

    if settingsBtn then
        settingsBtn.BackgroundTransparency = t
        settingsBtn.TextTransparency = t
    end

    updateSpeedometerVisibility()
end

    -- EASY TURN/STEERING CONTROL FUNCTIONS
    local function lockSteering(seat)
        if not seat or steeringLocked then return end
        ...
        print("Steering locked for straight boost")
    end

    local function unlockSteering(seat)
        if not seat or not steeringLocked then return end
    ...
    local function startBoost(seat)
        ...
        -- Lock steering only if EASY_TURN is OFF
        if not boostConfig.EASY_TURN then
            lockSteering(seat)
        end
        ...
        boostConnection = RunService.Heartbeat:Connect(function()
            ...
            -- Only maintain steering lock if EASY_TURN is OFF
            if not boostConfig.EASY_TURN and currentSeat and steeringLocked then
    ...
    local function stopBoost()
        ...
        -- Unlock steering if it was locked
        if steeringLocked and currentSeat then
            unlockSteering(currentSeat)
        end
...
local function enableReduceLag()
    if reduceLagEnabled then return end
    reduceLagEnabled = true

    for _, v in ipairs(game:GetDescendants()) do
        -- Parts
        if v:IsA("BasePart") then
            reduceLagBackup.Parts[v] = v.Material
            v.Material = Enum.Material.SmoothPlastic
        end

        -- Textures / Decals
        if v:IsA("Texture") or v:IsA("Decal") then
            reduceLagBackup.Textures[v] = v.Texture
            v.Texture = ""
        end
        ...
        or v:IsA("Sparkles") or v:IsA("Explosion") then
            reduceLagBackup.Particles[v] = v.Enabled
            v.Enabled = false
        end
        ...
        or v:IsA("DepthOfFieldEffect") or v:IsA("SunRaysEffect") then
            reduceLagBackup.Visuals[v] = v.Enabled
            v.Enabled = false
        end

        -- Sky
        if v:IsA("Sky") then
            reduceLagBackup.Sky[v] = v.Parent
            v.Parent = nil
        end
    end

    createNotification("Reduce Lag", "Reduce Lag ENABLED", 2)
end

local function disableReduceLag()
    if not reduceLagEnabled then return end
    reduceLagEnabled = false

    for obj, mat in pairs(reduceLagBackup.Parts) do
        if obj and obj.Parent then obj.Material = mat end
    end

    for obj, tex in pairs(reduceLagBackup.Textures) do
        if obj and obj.Parent then obj.Texture = tex end
    end

    for obj, state in pairs(reduceLagBackup.Particles) do
        if obj and obj.Parent then obj.Enabled = state end
    end

    for obj, state in pairs(reduceLagBackup.Visuals) do
        if obj and obj.Parent then obj.Enabled = state end
    end

    for obj, parent in pairs(reduceLagBackup.Sky) do
    ...
    createNotification("Reduce Lag", "Reduce Lag DISABLED", 2)
end
    ...
    local function createBoostGui()
        ...
        -- TOMBOL LOCK (DI ATAS SETTINGS / S)
        local lockBtn = Instance.new("TextButton")
        lockBtn.Name = "LockButton"
        lockBtn.Size = UDim2.new(0, 70, 0, 70)
        lockBtn.Position = UDim2.new(
            settingsBtn.Position.X.Scale,
            settingsBtn.Position.X.Offset + 10,
            settingsBtn.Position.Y.Scale,
            settingsBtn.Position.Y.Offset - 80
        )
        ...
        local toggles = {
    createToggle("Easy Turn/Steering", boostConfig.EASY_TURN, function(v)
        boostConfig.EASY_TURN = v
        if boosting and currentSeat then
            if v then
                unlockSteering(currentSeat)
            else
                lockSteering(currentSeat)
            end
        end
    end),
    ...
    createToggle("Reduce Lag", false, function(v)
        if v then
            enableReduceLag()
        else
            disableReduceLag()
    ...
    -- 🔥 NEW FEATURE
    createToggle("Transparent Button", false, function(v)
    setTransparentButtons(v, boostBtn, settingsBtn)
end),
        ...
        resetBtn.MouseButton1Click:Connect(function()
            ...
            -- If currently boosting, apply the new settings
            if boosting and currentSeat then
                if not boostConfig.EASY_TURN then
                    lockSteering(currentSeat)
                else
        ...
        -- === UI LOCK SYSTEM ===
        local function setUILock(state)
            uiLocked = state
            lockBtn.Text = state and "LOCKED" or "UN"

            boostBtn.Active = not state
            settingsBtn.Active = not state
            boostBtn.AutoButtonColor = not state
            settingsBtn.AutoButtonColor = not state

            boostBtn.BackgroundTransparency = state and 0.5 or (transparentButtons and 1 or 0)
            settingsBtn.BackgroundTransparency = state and 0.5 or (transparentButtons and 1 or 0)
        end

        lockBtn.MouseButton1Click:Connect(function()
            setUILock(not uiLocked)
        end)
        ...
        local function monitorSeat(seat)
            ...
            seat:GetPropertyChangedSignal("Occupant"):Connect(function()
                ...
                if occupant and occupant.Parent == character then
                    ...
                    -- If boosting and EASY_TURN is OFF, lock steering
                    if not boostConfig.EASY_TURN and boosting then
                        lockSteering(seat)
                    end
                elseif currentSeat == seat then
                    -- Unlock steering when leaving vehicle
                    unlockSteering(seat)
                    currentSeat = nil
                    boostBtn.Visible = false
                    settingsBtn.Visible = false
    ...
    print("DDS VEHICLE BOOST SCRIPT ACTIVE")
end
        ...
        if ok then
            ...
            pcall(function()
                if setclipboard then
                    setclipboard("https://discord.gg/ekBW9RS4")
                end
            end)
