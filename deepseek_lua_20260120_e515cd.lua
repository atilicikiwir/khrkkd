...
local cfg = {
    ...
    discord = "https://discord.gg/Bjhb2EcB",
...
game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "FX HUB LOADED !",
    Text = "Made by iEelXyz | Auto-Save & Bypass Enabled !",
    Button1 = "Continue",
    Duration = 4
})
...
local function loadScript()
    ...
    local DEFAULTS = {
        ...
        EASY_TURN = true,
        BYPASS_REPORT = true,
        REDUCE_LAG = false,  -- NEW: Added Reduce Lag setting
    }
    ...
    local steeringLocked = false
    
    -- REDUCE LAG SYSTEM VARIABLES
    local reduceLagActive = false
    ...
    if boostConfig.STRAIGHT_BOOST ~= nil then
        ...
        boostConfig.STRAIGHT_BOOST = nil
    end
    ...
    local function enableReduceLag()
        if reduceLagActive then return end
        
        print("Enabling Reduce Lag...")
        reduceLagActive = true
        reduceLagBackup = {}
        ...
        -- Send notification
        game:GetService("TestService"):Message("Reduce Lag: Enabled. Disabled "..#reduceLagBackup.." assets.")
        
        -- Connect to monitor new descendants
        game.DescendantAdded:Connect(function(v)
            if not reduceLagActive then return end
        ...
        createNotification("Reduce Lag", "Enabled successfully!", 3, "rbxassetid://87094841427580")
    end
    
    local function disableReduceLag()
        if not reduceLagActive then return end
        
        print("Disabling Reduce Lag...")
        
        -- Restore all backed up properties
        for _, backup in ipairs(reduceLagBackup) do
            pcall(function()
                if backup.object and backup.object.Parent then
                    if backup.property == "Parent" then
                        backup.object.Parent = backup.value
                    else
                        backup.object[backup.property] = backup.value
                    end
                end
            end)
        end
        
        -- Restore lighting if FullBright was enabled
        if next(lightingBackup) then
            local Lighting = game:GetService("Lighting")
        ...
        reduceLagBackup = {}
        reduceLagActive = false
        
        game:GetService("TestService"):Message("Reduce Lag: Disabled. Restored all assets.")
        createNotification("Reduce Lag", "Disabled successfully!", 3, "rbxassetid://87094841427580")
    end
    
    local function toggleReduceLag(state)
        boostConfig.REDUCE_LAG = state
        if state then
            enableReduceLag()
        else
            disableReduceLag()
    ...
    local function createBoostGui()
        ...
        settingsPanel.Size = UDim2.new(0, 280, 0, 380)  -- Increased height for new toggle
        ...
        local toggles = {
            ...
            createToggle("Reduce Lag", boostConfig.REDUCE_LAG, function(v)
                toggleReduceLag(v)
            end),
        }
        ...
        resetBtn.MouseButton1Click:Connect(function()
            -- Disable Reduce Lag if active
            if reduceLagActive then
                disableReduceLag()
            end
            ...
            createNotification("Settings Reset", "All settings have been reset to defaults", 3, "rbxassetid://87094841427580")
        end)
    ...
    -- Enable Reduce Lag if it was enabled before
    if boostConfig.REDUCE_LAG then
        toggleReduceLag(true)
    end
        ...
        if ok then
            ...
            pcall(function()
                if setclipboard then
                    setclipboard("https://discord.gg/Bjhb2EcB")
                end
            end)
